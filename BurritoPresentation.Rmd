---
title: "Burrito Analysis"
author: "Ben Prado, Samantha Bothwell, Jiawen Liu, Jose Amaya Santanach"
...
- The question that we are investigating is how do ingredients effect cost.
- We run two models one looking at meat ingredients and one with top 10 other ingredients.
- Location is used as a random effect.

## Data processing

- To prepare the data, we made two data sets. One data set looked at the meat ingredients (Beef, Pork, Chicken, Shrimp, Fish) and the top 10 other ingredients used (Pico, Guac, Cheese, Fries, Sour cream, Rice, Beans, Sauce, Cilantro, Onion)
- We removed two observations because they were not complete.
...
- We used a Block Gibbs sampler to estimate the values.


## Diagnostics for meat model
...
plot(meat$beta[,5],type = "l",las=1,main = "beta5 (Shrimp)")
```

- Convergence looks good.

## Diagnostics for other ingredients model
...
```{r,echo=FALSE}
par(mfrow=c(2,7))
plot(other$alpha,type = "l",las=1,main = "alpha")
plot(other$sigma,type = "l",las=1,main = "sigma")
plot(other$kappa,type = "l",las=1,main = "kappa")
plot(other$beta[,1],type = "l",las=1,main = "beta1 (Cheese)")
plot(other$beta[,2],type = "l",las=1,main = "beta2 (Pico)")
plot(other$beta[,3],type = "l",las=1,main = "beta3 (Guac)")
plot(other$beta[,4],type = "l",las=1,main = "beta4 (Fries)")
plot(other$beta[,5],type = "l",las=1,main = "beta5 (Sour_cream)")
plot(other$beta[,6],type = "l",las=1,main = "beta6 (Sauce)")
plot(other$beta[,7],type = "l",las=1,main = "beta7 (Rice)")
plot(other$beta[,8],type = "l",las=1,main = "beta8 (Beans)")
plot(other$beta[,9],type = "l",las=1,main = "beta9 (Onion)")
plot(other$beta[,10],type = "l",las=1,main = "beta10 (Cilantro)")
```
...
- Shrimp is significant for a higher price.
...
- Sauce is significant for a higher price.
- Beans are significant for a lower price.


## Conclusions

- We can see from the $\beta_i$ estimates that Shrimp and Sauce have an positive effect on price and Beans have a negative effect.

```{r,echo=FALSE}
locations=unique(burrito$Location)
```


```{r}
table(apply(meat$gamma,1,which.max))/1000
locations[47]
```

- The most expensive location is by far Humbertos.

## Conclusions cont.

- We can also look and see which restaurants are the "cheapest".
```{r}
...
- Overall the locations that have the lowest price are Pokirrito, Oscar's Mexican Food, and Raul's Mexican food.
